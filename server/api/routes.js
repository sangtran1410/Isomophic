...
function routes(router) {
  ...
  */
  ...
  //       dest: '/home/binh.nguyen/Documents/files'                  // Save to /path/to/dest/image.jpg 
  //     }
  ...
  router.post('/saveImageFromUrl', async function (ctx) {
    const promiseArr = []
    const p = new Promise((resolve) => {
      const url = 'https://images.pexels.com/photos/67636/rose-blue-flower-rose-blooms-67636.jpeg'
      const options = {
        // directory: './images',   //in project folder
        directory: './../images',   //out project folder
        filename: 'cassst.gif'
      }
...
<sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  ...
  router.post('/sharesocial/:type', async function (ctx) {
    ...
    if (type === 'all' || type === 'linkedin') {
      const pLinkedIn = SocialShare.shareLinkedinPromise();
      promiseArr.push(pLinkedIn)
    }
    /*Share Twitter*/
    if (type === 'all' || type === 'twitter') {
      const pTwitter = SocialShare.shareTwitterPromise();
      promiseArr.push(pTwitter)
    }
    /*Share Facebook page */
    if (type === 'all' || type === 'facebook') {
      const pFacebook = SocialShare.shareFBPromise();
      promiseArr.push(pFacebook)
    }

    /*Share Facebook wall */
    if (type === 'all' || type === 'facebook') {
      const pFacebook2 = SocialShare.shareFBWallPromise();
      promiseArr.push(pFacebook2)
    }
    /** start promise */
    await Promise.all(promiseArr).then(async (arr) => {
      ctx.body = arr
  ...
  router.post('/getprofile', async function (ctx) {
    const promiseArr = []
    promiseArr.push(SocialShare.profileFBPromise())
    promiseArr.push(SocialShare.profileTwitterPromise())
    promiseArr.push(SocialShare.profileLinkedinPromise())
